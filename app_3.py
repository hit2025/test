...
# -------------------------
# DRIVER DASHBOARD EXTENSION
# -------------------------
@app.get("/driver_status/{vehicle_id}")
def driver_status(vehicle_id: int):
    ...
    distance = haversine(v["lat"], v["lng"], b["lat"], b["lng"])
    return {
        "status": "ASSIGNED",
        "vehicle_id": v["id"],
        "vehicle_location": {"lat": v["lat"], "lng": v["lng"]},
        "assigned_bin": {"id": b["id"], "lat": b["lat"], "lng": b["lng"], "fill": b["fill"]},
        "distance": round(distance, 1),
        "instructions": f"Drive to Bin {b['id']} located at ({b['lat']}, {b['lng']})"
    }

@app.post("/driver_complete/{vehicle_id}")
def driver_complete(vehicle_id: int):
    v = next((veh for veh in vehicles if veh["id"] == vehicle_id), None)
    if not v or not v["target_bin"]:
        return {"ok": False, "message": "No active assignment"}
    bid = v["target_bin"]
    complete_trip(vehicle_id, bid)
    return {"ok": True, "message": f"Vehicle {vehicle_id} marked trip complete for Bin {bid}"}
